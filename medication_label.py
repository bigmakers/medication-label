...
def draw_label(c, facility, name, date, timing, use_hiragana=False, show_date=True, show_facility=True, name_reading=""):
    ...
    if name_reading:
        ...
        y_cursor -= 4 * mm
        c.drawCentredString(cx, y_cursor, name_reading)
...
class MedicationLabelApp:
    ...
    def create_widgets(self):
        # タイトル
        ttk.Label(self.root, text="💊 服薬ラベル発行", font=("", 18, "bold")).pack(pady=(15, 10))
        
        # ===== 患者リスト =====
        frame1 = ttk.LabelFrame(self.root, text="📋 患者リスト", padding=10)
        frame1.pack(fill=tk.X, padx=15, pady=5)
        
        row1 = ttk.Frame(frame1)
        row1.pack(fill=tk.X)
        
        self.patient_combo = ttk.Combobox(row1, state="readonly", width=30, font=("", 12))
        ...
        ttk.Button(row1, text="💾 保存", command=self.save_patient).pack(side=tk.LEFT, padx=(10, 2))
        ttk.Button(row1, text="🗑️ 削除", command=self.delete_patient).pack(side=tk.LEFT, padx=2)
        
        # ===== 基本情報 =====
        frame2 = ttk.LabelFrame(self.root, text="👤 基本情報", padding=10)
        frame2.pack(fill=tk.X, padx=15, pady=5)
        
        row2 = ttk.Frame(frame2)
        row2.pack(fill=tk.X)

        ttk.Label(row2, text="施設名:", font=("", 12)).pack(side=tk.LEFT)
        self.facility_entry = ttk.Entry(row2, width=15, font=("", 12))
        self.facility_entry.pack(side=tk.LEFT, padx=(5, 20))

        ttk.Label(row2, text="氏名:", font=("", 12)).pack(side=tk.LEFT)
        self.name_entry = ttk.Entry(row2, width=15, font=("", 12))
        self.name_entry.pack(side=tk.LEFT, padx=5)

        row2b = ttk.Frame(frame2)
        row2b.pack(fill=tk.X, pady=(5, 0))

        ttk.Label(row2b, text="よみがな:", font=("", 12)).pack(side=tk.LEFT)
        self.reading_entry = ttk.Entry(row2b, width=15, font=("", 12))
        self.reading_entry.pack(side=tk.LEFT, padx=(5, 0))
        ttk.Label(row2b, text="例: やまだ たろう", foreground="gray").pack(side=tk.LEFT, padx=10)
        
        # ===== 印刷設定 =====
        frame3 = ttk.LabelFrame(self.root, text="🖨️ 印刷設定", padding=10)
        frame3.pack(fill=tk.X, padx=15, pady=5)
        
        # 印刷順序
        row3a = ttk.Frame(frame3)
        row3a.pack(fill=tk.X, pady=5)
        
        ttk.Label(row3a, text="印刷順序:", font=("", 12)).pack(side=tk.LEFT)
        self.sort_var = tk.StringVar(value="timing")
        ttk.Radiobutton(row3a, text="まとめ印刷", variable=self.sort_var, value="timing").pack(side=tk.LEFT, padx=10)
        ...
        ttk.Label(row3b, text="用法表示:", font=("", 12)).pack(side=tk.LEFT)
        self.hiragana_var = tk.StringVar(value="kanji")
        ttk.Radiobutton(row3b, text="漢字（朝・昼・夕）", variable=self.hiragana_var, value="kanji").pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(row3b, text="ひらがな（あさ・ひる・ゆう）", variable=self.hiragana_var, value="hiragana").pack(side=tk.LEFT)
        
        # 表示オプション
        row3b2 = ttk.Frame(frame3)
        row3b2.pack(fill=tk.X, pady=5)

        self.show_facility_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(row3b2, text="施設名を印刷する", variable=self.show_facility_var).pack(side=tk.LEFT, padx=(0, 15))

        self.show_date_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(row3b2, text="日付を印刷する", variable=self.show_date_var).pack(side=tk.LEFT)
        
        # 日付
        row3c = ttk.Frame(frame3)
        row3c.pack(fill=tk.X, pady=10)
        
        ttk.Label(row3c, text="開始日:", font=("", 12)).pack(side=tk.LEFT)
        ...
        ttk.Label(row3c, text="日数:", font=("", 12)).pack(side=tk.LEFT, padx=(20, 0))
        self.days_var = tk.StringVar(value="7")
        ttk.Spinbox(row3c, from_=1, to=365, width=5, textvariable=self.days_var, font=("", 12)).pack(side=tk.LEFT, padx=5)
        ttk.Label(row3c, text="日分", font=("", 12)).pack(side=tk.LEFT)
        
        # 服用時点
        ttk.Label(frame3, text="服用時点:", font=("", 12)).pack(anchor=tk.W, pady=(10, 5))
        ...
        for i, (timing, default) in enumerate(timings):
            ...
            cb.grid(row=i // 4, column=i % 4, sticky=tk.W, padx=8, pady=3)
        
        # カスタム
        row3d = ttk.Frame(frame3)
        row3d.pack(fill=tk.X, pady=(10, 0))
        
        ttk.Label(row3d, text="カスタム:", font=("", 12)).pack(side=tk.LEFT)
        self.custom_entry = ttk.Entry(row3d, width=20, font=("", 12))
        ...
        print_btn = tk.Button(
            ...
            font=("", 16, "bold"), bg="#2563eb", fg="white",
            activebackground="#1d4ed8", activeforeground="white",
            pady=12, command=self.generate_labels
        )
        print_btn.pack(fill=tk.X, padx=15, pady=20)
        
        # ===== データ管理 =====
        data_frame = ttk.Frame(self.root)
        data_frame.pack(fill=tk.X, padx=15)
    ...
    def update_patient_list(self):
        self.patients = load_patients()
        self.patients.sort(key=lambda p: p.get("name", ""))
        values = ["-- 新規入力 --"] + [
